...
FASTQ2SAM() {
  local FASTQ_PFX="$1"
  local BAM_WRITERS="${2:-4}"
  local SAMPLE="$(basename $FASTQ_PFX "_")"
  \time \
    -f '==========\ntime: %esec (system:%S user:%U %P)\nmaxmem: %Mkb\ninput:%I (%r sockets) output:%O (%s sockets)\ncommand: %C\nexit: %x' \
    $NH/Projects/Fastq2Sam/build/fastq2sam -1 "${FASTQ_PFX}1.fastq.gz" -2 "${FASTQ_PFX}2.fastq.gz" -o "$SAMPLE.bam" -n "$SAMPLE" --bam-writers "$BAM_WRITERS"
...
for I in {1..8}; do FASTQ2SAM "/work28/public_WGS/ENA/PRJEB11455/ERR1813570_" $I; done
